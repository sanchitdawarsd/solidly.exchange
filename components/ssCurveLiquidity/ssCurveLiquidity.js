...
import classes from './ssCurveLiquidity.module.css';
...
export default function SSCurveLiquidity({ asset }) {
  ...
  useEffect(() => {
    ...
    stores.emitter.on(ACTIONS.FIXED_FOREX_DEPOSIT_CURVE_APPROVED, approveReturned);
    stores.emitter.on(ACTIONS.FIXED_FOREX_CURVE_DEPOSITED, depositReturned);
    stores.emitter.on(ACTIONS.FIXED_FOREX_CURVE_WITHDRAWN, depositReturned);
    stores.emitter.on(ACTIONS.FIXED_FOREX_SLIPPAGE_INFO_RETURNED, slippageInfoReturned);
    stores.emitter.on(ACTIONS.ERROR, errorReturned);
    return () => {
      stores.emitter.removeListener(ACTIONS.FIXED_FOREX_DEPOSIT_CURVE_APPROVED, approveReturned);
      stores.emitter.removeListener(ACTIONS.FIXED_FOREX_CURVE_DEPOSITED, depositReturned);
      stores.emitter.removeListener(ACTIONS.FIXED_FOREX_CURVE_WITHDRAWN, depositReturned);
      stores.emitter.removeListener(ACTIONS.FIXED_FOREX_SLIPPAGE_INFO_RETURNED, slippageInfoReturned);
  ...
  const setAmountPercent = (input, percent) => {
    if(input === 'amount0') {
      ...
      setAmount0(am);
      stores.dispatcher.dispatch({ type: ACTIONS.FIXED_FOREX_GET_SLIPPAGE_INFO, content: { asset, amount0: am, amount1 } })

    } else if (input === 'amount1') {
      let am = BigNumber(asset.gauge.coin1.balance).times(percent).div(100).toFixed(asset.gauge.coin1.decimals)
      setAmount1(am);
      stores.dispatcher.dispatch({ type: ACTIONS.FIXED_FOREX_GET_SLIPPAGE_INFO, content: { asset, amount0, amount1: am } })
  ...
  const onApprove = (idx) => {
    ...
    stores.dispatcher.dispatch({ type: ACTIONS.FIXED_FOREX_APPROVE_DEPOSIT_CURVE, content: { asset, coin, amount } })
  }

  const onDeposit = () => {
    setDepositLoading(true)
    stores.dispatcher.dispatch({ type: ACTIONS.FIXED_FOREX_DEPOSIT_CURVE, content: { asset, amount0, amount1 } })
  }

  const onWithdraw = () => {
    setDepositLoading(true)
    stores.dispatcher.dispatch({ type: ACTIONS.FIXED_FOREX_WITHDRAW_CURVE, content: { asset, withdrawAmount, withdrawAmount0, withdrawAmount1 } })
  }
